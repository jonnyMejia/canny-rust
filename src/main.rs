...
pub fn canny<T: Into<image::GrayImage>>(
    ...
    //print!("{:?}",gs_image.pixels());
    //for p in gs_image.pixels() { println!("pixel: {:?}", p)};
...
fn main() {
    ...
    let img = image::open("ruido.jpeg").expect("failed to read image").to_luma();;
    ...
    detection.as_image().save("Resultado.png");
